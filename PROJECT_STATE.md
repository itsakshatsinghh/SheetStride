...
*   **Interview Notebook & LeetCode Description Drawer:** Private structured study journal (Brute Force, Optimization path, Pattern Strategy, Dry Run) and timeline log viewer inside right-hand slidedrawer. Added support for rendering interactive problem descriptions, collapsible system hints, likes/dislikes metrics, direct LeetCode attempt links, and dynamic back-navigation headers.
*   **Submission Calendar HUD:** Interactive dark-themed calendar grid embedded on the Progress page representing solved dates and due revisions. Clickable day blocks display solved checklist logs and future revision queue cards, complete with distinguishability cyber yellow borders and background tints.
